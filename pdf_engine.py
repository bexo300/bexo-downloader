...
class PDFEngine:
    ...
    def add_page_numbers(pdf_path: str) -> str:
        try:
            ...
            for i, page in enumerate(doc, 1):
                rect = fitz.Rect(page.rect.width * 0.45, page.rect.height - 40, page.rect.width * 0.55, page.rect.height - 10)
                page.insert_textbox(rect, str(i), fontsize=14, color=(0, 0, 0), align=fitz.TEXT_ALIGN_CENTER)
    ...
    def add_watermark(pdf_path: str, text: str) -> str:
        ...
        try:
            ...
            for page in doc:
                rect = page.rect
                page.insert_textbox(fitz.Rect(rect.width * 0.15, rect.height * 0.35, rect.width * 0.85, rect.height * 0.65), text, fontsize=48, color=(0.6, 0.6, 0.6, 0.25), align=fitz.TEXT_ALIGN_CENTER)
                page.insert_textbox(fitz.Rect(rect.width * 0.02, rect.height * 0.92, rect.width * 0.98, rect.height * 0.98), text, fontsize=10, color=(0.7, 0.7, 0.7, 0.4), align=fitz.TEXT_ALIGN_CENTER)
    ...
    def images_to_pdf(image_paths: List[str]) -> str:
        ...
        try:
            ...
            if len(images) == 1:
                ...
                images[0].save(str(out_path), "PDF", save_all=True, append_images=images[1:], resolution=100.0)
            for img in images:
                img.close()
            return str(out_path)
        except Exception as e:
